...
include (MINIBB_PATH.'setup_'.$DB.'.php');
include (MINIBB_PATH.'bb_codes.php');
include (MINIBB_PATH.'bb_functions.php');
include (MINIBB_PATH.'bb_specials.php');
...
if (isset($_POST['mode']) and $_POST['mode']=='login') require(MINIBB_PATH.'bb_func_login.php');
...
user_logged_in();
if($user_id!=0 and isset($langu) and $langu=str_replace(array('.','/','\\'),'',$langu) and file_exists(MINIBB_PATH."lang/{$langu}.php")) $lang=$langu;
elseif($user_id==0 and isset($_GET['setlang']) and $setlang=str_replace(array('.','/','\\'),'',$_GET['setlang']) and file_exists(MINIBB_PATH."lang/{$_GET['setlang']}.php")) {$lang=$setlang; $indexphp.='setlang='.$setlang.'&';}

include (MINIBB_PATH."lang/$lang.php");
...
include (MINIBB_PATH.'bb_plugins.php');

/* Main actions */
$backErrorLink="<a href=\"JavaScript:history.back(-1)\">$l_back</a>";

if($action=='pthread') {if($reqTxt!=1)require(MINIBB_PATH.'bb_func_txt.php');require(MINIBB_PATH.'bb_func_pthread.php');}
elseif($action=='ptopic') {if($reqTxt!=1)require(MINIBB_PATH.'bb_func_txt.php');require(MINIBB_PATH.'bb_func_ptopic.php');}
...
elseif($action=='vthread') require(MINIBB_PATH.'bb_func_vthread.php');
...
else require(MINIBB_PATH.'bb_func_vtopic.php');
}
...
elseif($action=='search') {if($reqTxt!=1)require(MINIBB_PATH.'bb_func_txt.php');require(MINIBB_PATH.'bb_func_search.php');}

elseif($action=='deltopic') require(MINIBB_PATH.'bb_func_deltopic.php');

elseif($action=='locktopic') require(MINIBB_PATH.'bb_func_locktop.php');

elseif($action=='editmsg') {$step=0;require(MINIBB_PATH.'bb_func_editmsg.php');}

elseif($action=='editmsg2') {require(MINIBB_PATH.'bb_func_txt.php');$step=1;require(MINIBB_PATH.'bb_func_editmsg.php');}

elseif($action=='delmsg') {$step=0;require(MINIBB_PATH.'bb_func_delmsg.php');}

elseif($action=='movetopic') {$step=0;require(MINIBB_PATH.'bb_func_movetpc.php');}

elseif($action=='movetopic2') {$step=1;require(MINIBB_PATH.'bb_func_movetpc.php');}

elseif($action=='userinfo') require(MINIBB_PATH.'bb_func_usernfo.php');

elseif($action=='sendpass') {$step=0;require(MINIBB_PATH.'bb_func_sendpwd.php');}

elseif($action=='sendpass2') {$step=1;require(MINIBB_PATH.'bb_func_sendpwd.php');}

elseif($action=='confirmpasswd') {if (!isset($confirmCode)) $confirmCode=''; require(MINIBB_PATH.'bb_func_confpwd.php');}

elseif($action=='stats') require(MINIBB_PATH.'bb_func_stats.php');

elseif($action=='manual') require(MINIBB_PATH.'bb_func_man.php');

elseif($action=='registernew' and $enableNewRegistrations) {$step=0;require(MINIBB_PATH.'bb_func_regusr.php');}

elseif($action=='register' and $enableNewRegistrations) {$step=1;require(MINIBB_PATH.'bb_func_regusr.php');}

elseif($action=='prefs' and $enableProfileUpdate) {$step=0;require(MINIBB_PATH.'bb_func_editprf.php');}

elseif($action=='editprefs' and $enableProfileUpdate) {$step=1;require(MINIBB_PATH.'bb_func_editprf.php');}

elseif($action=='unsubscribe') require(MINIBB_PATH.'bb_func_unsub.php');

elseif($action=='sticky') {$status=9;require(MINIBB_PATH.'bb_func_sticky.php');}

elseif($action=='unsticky') {$status=0;require(MINIBB_PATH.'bb_func_sticky.php');}

elseif($action=='viewipuser') {require(MINIBB_PATH.'bb_func_viewip.php');}

elseif($action=='poll') {require(MINIBB_PATH.'hack_poll.php');}

elseif($action=='tpl') {
if (isset($tplName) and $tplName!='' and file_exists (MINIBB_PATH.'templates/'.$tplName.'.html')){
echo load_header();
if(file_exists(MINIBB_PATH.'templates/'.$tplName.'.html'))
...
require(MINIBB_PATH.'bb_func_vforum.php');
if ($viewlastdiscussions!=0) {
require(MINIBB_PATH.'bb_func_ldisc.php');
