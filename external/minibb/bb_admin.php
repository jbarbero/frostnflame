...
include (MINIBB_PATH.'setup_options.php');
...
$iconList='';
if($handle=@opendir(MINIBB_PATH.'img/forum_icons')) {
$ss=0;
while (($file=readdir($handle))!==false) {
if ($file != "." && $file != "..") {
$iconList.="<a href=\"JavaScript:paste_strinL('{$file}')\" onMouseOver=\"window.status='{$GLOBALS['l_forumIcon']}: {$file}'; return true\"><img src=\"".MINIBB_PATH."img/forum_icons/{$file}\" border=0 alt=\"{$file}\"></a>&nbsp;&nbsp;";
$ss++;
if ($ss==5) {
$ss=0;
$iconList.="<br>\n";
...
$fast='';
for ($i=0; $i<=$count; $i++) {
$fast.="<img src=\"".MINIBB_PATH."img/forum_icons/".$resultVal["forum_icon"][$i]."\" width=16 height=16 border=0 alt=\"Forum icon\">&nbsp;<b><a href={$GLOBALS['bb_admin']}action=editforum2&amp;forumID=".$resultVal["forum_id"][$i].">".$resultVal["forum_name"][$i]."</a></b> [ORDER: ".$resultVal["forum_order"][$i]."] - <i><span class=txtSm>".$resultVal["forum_desc"][$i]."</span></i><br>";
}
...
$user_id=0;
user_logged_in();
if(isset($langu) and file_exists(MINIBB_PATH."lang/{$langu}.php")) $lang=$langu;
include (MINIBB_PATH."lang/$lang.php");
...
if (file_exists(MINIBB_PATH."img/forum_icons/{$forumicon}")) {
